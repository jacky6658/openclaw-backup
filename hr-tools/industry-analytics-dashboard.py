#!/usr/bin/env python3
"""
Industry Analytics Dashboard - 產業分析儀表板
...
class AnalyticsDashboard:
    ...
    def _generate_html_dashboard(self, metrics: DashboardMetrics, 
        ...
        # Pre-serialize JSON for embedding in f-string
...
<body>
    <div class="container">
        ...
        <div class="metrics-grid">
            ...
            <div class="metric-card alt3">
                ...
                <div class="metric-value">{metrics.top_industries[0][0] if metrics.top_industries else 'N/A'}</div>
            </div>
        </div>
        ...
        for candidate in candidates_data.get('candidates', [])[:10]:
            ...
            html += f"""
                    <tr>
                        ...
                        <td>{candidate.get('source_industry', 'N/A').upper()}</td>
    ...
    <script>
        ...
        # 添加圖表數據
        html += ', '.join([f"'{ind}'" for ind in industry_labels])
        html += f"""],
                datasets: [{{
                    data: [{', '.join(map(str, industry_values))}],
                    backgroundColor: [{', '.join([f"'{color}'" for color in industry_colors])}],
                    borderColor: '#fff',
                    borderWidth: 2
                }}]
            }},
...
def main():
    ...
    metrics = dashboard.generate_dashboard(candidates_data, jobs_data)
