...
class CandidateMatcher:
    def __init__(self):
        # 評分權重（v3 更新：加入穩定性維度）
        self.weights = {
            'skill_match': 0.30,      # 技能匹配度 30% (從 40% 降低)
            'experience': 0.20,       # 經驗年資 20% (從 30% 降低)
            'industry': 0.10,         # 產業相關性 10% (從 20% 降低)
            'stability': 0.30,        # 穩定性預測 30% (新增) ✨
            'bonus': 0.10             # 其他加分項 10%
        }
        ...
        # 紅旗扣分項
        self.red_flags = {
            'frequent_job_hopping': -15,   # 頻繁跳槽（已由穩定性預測覆蓋）
    ...
    def normalize_skill(self, skill: str) -> str:
        ...
        synonyms = {
            ...
            'k8s': 'kubernetes',
            'ml': 'machine learning'
            # 移除 'ai': 'artificial intelligence' 映射（與模糊匹配衝突）
        }
    ...
    def calculate_skill_match(self, candidate_skills: List[str], jd_required: List[str], jd_preferred: List[str] = None) -> Tuple[float, dict]:
        ...
        # 必備技能匹配（模糊匹配：substring or reverse substring）
        def fuzzy_match(jd_skill, candidate_skills):
            """檢查 JD 技能是否匹配任一候選人技能"""
            jd_lower = jd_skill.lower()
            for c_skill in candidate_skills:
                c_lower = c_skill.lower()
                # substring matching: "ai" in "ai engineer" or "ai engineer" in "ai"
                if jd_lower in c_lower or c_lower in jd_lower:
                    return True
            return False
        
        required_matches = sum(1 for skill in jd_required_norm if fuzzy_match(skill, candidate_skills_norm))
        required_ratio = required_matches / len(jd_required_norm) if jd_required_norm else 0
        
        # 加分技能匹配（同樣使用模糊匹配）
        preferred_matches = sum(1 for skill in jd_preferred_norm if fuzzy_match(skill, candidate_skills_norm))
    ...
    def match(self, candidate: Dict, jd: Dict) -> Dict:
        ...
        # 4. 穩定性預測（新增 v3）✨
        stability_result = self.stability_predictor.predict(candidate, mode='auto')
        # 將穩定性 0-100 分轉換為配對系統的 0-30 分
        stability_raw_score = stability_result['score']  # 0-100
        stability_score = (stability_raw_score / 100) * 30  # 轉換為 0-30 分
        stability_details = stability_result
        ...
        # 總分計算
        total_score = skill_score + experience_score + industry_score + stability_score + bonus_score + penalty
        ...
        result = {
            ...
            'breakdown': {
                ...
                'stability': round(stability_score, 1),  # 新增 v3 ✨
            ...
            'details': {
                ...
                'stability': stability_details,  # 新增 v3 ✨
                'bonus': bonus_details,
                'red_flags': red_flags
            },
