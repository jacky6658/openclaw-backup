...
### 5.1 專案 skills vs 全域 Clawdbot skills

- 專案內 skills：`/Users/user/clawd/skills`  
  - 例如：`agent-browser/`、`ai-pdf-builder/`、`oracle/`。  
  - 跟這個 workspace 綁定，適合放你自己寫的技能。  
- 全域 Clawdbot skills：`.../node_modules/clawdbot/skills`  
  - 例如：`filesystem/`、`adaptive-suite/`、`weather/`、`trello/`。  
  - 由 `npm` 或 `clawdhub install` 安裝，給整個 Clawdbot 使用。  

建議：
- 從 Clawdhub 下載的技能（像 filesystem、adaptive-suite）放在全域 skills。  
- 專案特有或實驗性的技能，放在 `/Users/user/clawd/skills`。  
