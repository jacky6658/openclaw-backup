...
- 目標：
  ...
  - 未來接到專案板（Google Sheets / Trello / Notion / Jira 等）
- 目前狀態：
  - 有 trello、notion skills，但還沒定義「老闆怎麼講話 → 變成卡片／任務」的流程
- 下一步：
  - 先用 md 或 Google Sheet 做一個「一人公司老闆的 To-Do MVP」（已完成 today.md）
  - 預留「外部同步模板」：之後只要提供 Google Sheet ID 或 Trello / Notion API 設定，就可以把任務同步出去
